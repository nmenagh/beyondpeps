...
alter table if exists public.order_items
  add column if not exists price_cents_at_purchase integer not null default 0;

alter table if exists public.order_items
  add column if not exists product_title text not null default '';

alter table if exists public.order_items
  add column if not exists product_type text not null default 'Research Supplies';
