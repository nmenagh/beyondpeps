create table if not exists public.orders (
  ...
  shipping_method jsonb not null default '{}',
...
alter table if exists public.orders add column if not exists shipping_cents integer not null default 0;
alter table if exists public.orders add column if not exists shipping_address jsonb not null default '{}';
alter table if exists public.orders add column if not exists billing_address jsonb not null default '{}';
alter table if exists public.orders add column if not exists shipping_method jsonb not null default '{}';
alter table if exists public.orders add column if not exists tracking_number text;
alter table if exists public.orders add column if not exists tracking_url text;
alter table if exists public.orders add column if not exists shipping_provider text;
alter table if exists public.orders add column if not exists shipping_service text;
...
begin
  ...
  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'orders' and policyname = 'Admins manage orders') then
    create policy "Admins manage orders" on public.orders for all
      to authenticated using (public.beyond_peps_is_admin()) with check (public.beyond_peps_is_admin());
  end if;
  ...
  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'order_items' and policyname = 'Admins manage order items') then
...
begin
  ...
  loop
    ...
    select id, slug, name, title, price_cents, inventory_count
      into v_product
  ...
  values (
    ...
    jsonb_build_object('customer', coalesce(p_customer, '{}'::jsonb), 'paymentMethod', 'zelle')
  )
  ...
  loop
    ...
    insert into public.order_items (order_id, product_id, product_slug, product_name, quantity, unit_price_cents, total_cents)
    values (
      v_order_id,
      v_product.id,
      v_product.slug,
